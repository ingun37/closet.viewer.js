...
class MarkerManager {
  constructor({scene, camera, renderer, controls, updateRenderer}) {
    this.scene = scene;
    this.camera = camera;
  ...
  animateCamera(item) {
    ...
    if (this.camera.position.x !== to.x || this.camera.position.y !== to.y || this.camera.position.z !== to.z) {
      const onUpdate = () => {
        ...
        const interpolationTarget = new THREE.Vector3();
      ...
      const tween = TweenMax.to(position, totalTime, {
        x: to.x,
        y: to.y,
        z: to.z,
...
function Marker(pointerPosition, normal, cameraPosition, cameraTarget, cameraQuaternion, _message, _sprite) {
