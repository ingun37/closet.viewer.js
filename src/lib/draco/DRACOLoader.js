...
THREE.DRACOLoader.prototype = {
  ...
  decodeDracoFile: (function () {
    ...
    if (typeof DracoModule === "function") {
      dracoDecoder = DracoModule({ ENVIRONMENT: "WEB" });
    } else {
    ...
    return function (rawBuffer) {
      ...
      return scope.convertDracoGeometryTo3JS(
        wrapper,
        geometryType,
        buffer,
        dracoDecoder
  ...
  convertDracoGeometryTo3JS: function (
    ...
    const posAttId = wrapper.GetAttributeId(
      dracoGeometry,
    ...
    const colorAttId = wrapper.GetAttributeId(
      dracoGeometry,
      dracoDecoder.COLOR
    );
    ...
    const normalAttId = wrapper.GetAttributeId(
      dracoGeometry,
      dracoDecoder.NORMAL
    );
    ...
    let numOfUVs = 0;
    const texCoordAttId = wrapper.GetAttributeId(
      dracoGeometry,
    ...
    if (texCoordAttId != -1) {
      ...
      const texCoordAttribute = wrapper.GetAttribute(
        dracoGeometry,
        texCoordAttId
      );
    ...
    let uv2Id = texCoordAttId + 1;

    let uv2Data;

    if (
      (normalAttId == -1 && numAttributes == 3) ||
      (normalAttId != -1 && numAttributes == 4)
    ) {
      const uv2Attribute = wrapper.GetAttribute(dracoGeometry, uv2Id);
      if (uv2Attribute !== null) {
        uv2Data = new dracoDecoder.DracoFloat32Array();
        wrapper.GetAttributeFloatForAllPoints(
          dracoGeometry,
          uv2Attribute,
          uv2Data
    ...
    const geometryBuffer = {
      ...
      uv2s: new Float32Array(numTextureCoordinates),
    ...
    if (uv2Id != -1) {
      for (let i = 0; i < numTextureCoordinates; i += 2) {
        geometryBuffer.uv2s[i] = uv2Data.GetValue(i);
        geometryBuffer.uv2s[i + 1] = uv2Data.GetValue(i + 1);
      }
    }
    ...
    if (uv2Id != -1) dracoDecoder.destroy(uv2Data);
