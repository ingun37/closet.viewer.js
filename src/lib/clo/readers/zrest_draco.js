...
const getDracoGeometry = async (zrest, qsDracoFileName, JSZipOrDracoData) => {
    // Draco Compression
    ...
    const drcArrayBuffer = zrest.zProperty.bDisassembled
...
const buildStyleLines = (zrest, dracoGeometry, patternIdx, listLine, frontVertexCount) => {
    if (!listLine) {
        return;
    }
    ...
    for (let k = 0; k < listLine.length; ++k) {
        ...
        zrest.styleLineMap.set(patternIdx, currentStyleLineSet);
    }
};
...
const splitMatSpaceToMatMesh = async (
    ...
    const zrestVersion = zrest.zProperty.version;
    ...
    for (let m = 0; m < listIdxCount.length; ++m) {
         ...
         */
            ...
            // TODO: refactor this
        const bTransparent =
                matProperty.colorwayMaterials[zrest.colorwayIndex].bTransparent;
        ...
        zrest.matMeshMap.set(matMeshID, threeMesh);
...
export const createMatMesh = async (zrest, listMatShape, JSZipOrDracoData, tf, bLoadTransparentObject, materialInformationMap) => {

    let frontVertexCount = 0;

    const newArray = listMatShape.map(async (item, i) => {
        ...
        const dracoGeometry = await getDracoGeometry(zrest, dracoFilename, JSZipOrDracoData);
        ...
        if (listLine) {
            buildStyleLines(zrest, dracoGeometry, firstMatMeshID, listLine, frontVertexCount);
        }
    })
