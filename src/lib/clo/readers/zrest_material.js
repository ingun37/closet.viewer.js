...
import { loadTextureFile } from "@/lib/clo/readers/zrest_texture";
...
export async function makeMaterial({
  ...
  const colorwayMaterialsArray = property.colorwayMaterials;
  const colorwayMaterial = colorwayMaterialsArray[colorwayIndex];
  ...
  await loadZrestTexture(colorwayIndex);
  ...
  function attachShader(drawMode, version) {
  ...
  async function loadZrestTexture(colorwayIndex) {
    // Load Texture File
  ...
  function setTexturePropertiesFromMaterial(threeJSTexture, textureFileInfo) {
    ...
    rotMatrix.identity();
    rotMatrix.makeRotationZ(-THREE.Math.degToRad(textureFileInfo.angle));

    const transMatrix = new THREE.Matrix4();
    transMatrix.identity();
    transMatrix.makeTranslation(-textureFileInfo.translate.x, -textureFileInfo.translate.y, 0.0);

    const scaleMatrix = new THREE.Matrix4();
    scaleMatrix.identity();
    scaleMatrix.makeScale(1.0 / textureFileInfo.scale.x, 1.0 / textureFileInfo.scale.y, 1.0);
  ...
  function getUniforms() {
    ...
    if (version <= 2) {
      return {
        ...
        materialAmbient: buildV3Value(colorwayMaterial.ambient),
        materialDiffuse: buildV3Value(colorwayMaterial.diffuse),
        materialSpecular: buildV3Value(colorwayMaterial.specular),
        materialEmission: buildV3Value(colorwayMaterial.emission),
        materialShininess: buildFValue(colorwayMaterial.shininess),
        materialOpacity: buildFValue(colorwayMaterial.alpha),
        normalMapIntensityInPercentage: buildFValue(colorwayMaterial.normalMapIntensityInPercentage)
      };
    } else {
      // version > 3
      return {
        m_bUseMetalnessRoughnessPBR: buildTypeValue("i", colorwayMaterial.bUseMetalnessRoughnessPBR),
        m_Metalness: buildFValue(colorwayMaterial.metalness),
        m_Glossiness: buildFValue(colorwayMaterial.glossiness),
        ...
        m_EnvironmentLightIntensity: buildFValue(colorwayMaterial.environmentLightIntensity),
        m_CameraLightIntensity: buildFValue(colorwayMaterial.cameraLightIntensity),
        m_ReflectionIntensity: buildFValue(colorwayMaterial.reflectionIntensity),
        m_RoughnessUIType: buildTypeValue("i", colorwayMaterial.roughnessUIType),
        m_FrontColorMult: buildFValue(colorwayMaterial.frontColorMult),
        m_SideColorMult: buildFValue(colorwayMaterial.sideColorMult),

        materialBaseColor: buildV3Value(colorwayMaterial.base),
        materialSpecular: buildV3Value(colorwayMaterial.reflectionColor),
        materialOpacity: buildFValue(colorwayMaterial.alpha),
        normalMapIntensityInPercentage: buildFValue(colorwayMaterial.normalMapIntensityInPercentage),
