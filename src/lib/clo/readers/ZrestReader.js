...
function meshFactory(map, zip, retObject, loadedCamera) {
    ...
    if (colorways === undefined) {
        ...
        Global._globalCurrentColorwayIndex = colorways.get("uiCurrentCoordinationIndex");
        Global._globalColorwaySize = colorways.get("listColorway").length;
    }
    ...
    Global._globalMaterialInformationMap = new Map();
    ...
    if (zRestMatMeshArray === undefined) {
        ...
        for (var i = 0 ; i < zRestMatMeshArray.length ; ++i) {
            ...
            Global._globalMaterialInformationMap.set(id, zRestColorwayMaterials);
        }
    }
    ...
    var tf = GetMatMeshs(geometry, zip, false, version);
    retObject.add(tf);

    // 투명한것 추가
    tf = GetMatMeshs(geometry, zip, true, version);
    retObject.add(tf);

    return retObject;
}

function LoadTexture(zip, textureFileName)
{
        ...
        if (!zip.file(zRestTexture.file)) {
            ...
            else
            {
                ...
                if (!texture) {
                    ...
                    texture = LoadTexture(zip, textureFileName);
...
function AddMatMeshList(zip, listMatShape, tf, bLoadTransparentObject, version) {
    ...
    for (let i = 0 ; i < listMatShape.length ; ++i) {
        ...
        for (var m = 0; m < listIndexCount.length; ++m) {
            ...
            var matProperty = Global._globalMaterialInformationMap.get(matMeshID);
            var indexSize = listIndexCount[m];
            
            if (matProperty.colorwayMaterials[Global._globalCurrentColorwayIndex].bPerfectTransparent) {
            ...
            {
                if (!matProperty.colorwayMaterials[Global._globalCurrentColorwayIndex].bTransparent) {
                  //  indexOffset = indexOffset - listIndexCount[m + 1];
                    continue;
                }
            }
            ...
            var material = makeMaterialForZrest(zip, matProperty, Global._globalCurrentColorwayIndex, dracoGeometry.numUVs >= 2, version);
...
function GetMatMeshs(map, zip, bLoadTransparentObject, version) {
    ...
    if (listChildrenTransformer3D !== undefined && listChildrenTransformer3D !== null) {
        for (let i = 0; i < listChildrenTransformer3D.length; ++i) {
            ...
            var childTF = GetMatMeshs(childTF3D, zip, bLoadTransparentObject, version);
            tf.add(childTF);
        }
    }
    ...
    if (mapTransformer3D !== undefined && mapTransformer3D !== null) {
        var childTF = GetMatMeshs(mapTransformer3D, zip, bLoadTransparentObject, version);
        //tf.add(childTF);        
        tf = childTF;
    }
