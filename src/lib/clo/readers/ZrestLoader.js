...
export default function ZRestLoader({scene, camera, controls, cameraPosition}, manager) {
  ...
  this.meshFactory = new MeshFactory({
    ...
    matShapeList: this.matShapeList,
  },
...
ZRestLoader.prototype = {
  ...
  async makeMaterialForZrest(zip, matProperty, number, bUseSeamPuckeringNormalMap, camera, version) {
    return await makeMaterial(zip, matProperty, number, bUseSeamPuckeringNormalMap, camera, _drawMode, _seamPuckeringNormalMap, _nameToTextureMap, version);
  },
    ...
    reader.onload = (e) => {
      ...
      this.jsZip.loadAsync(e.target.result).then( (zip) => {
        ...
        zip.file(restName).async('arrayBuffer').then( async (restContent) => {
          ...
          _seamPuckeringNormalMap = await loadTexture(zip, 'seam_puckering_2ol97pf293f2sdk98.png');
