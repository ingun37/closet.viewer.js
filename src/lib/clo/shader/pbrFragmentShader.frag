    ...
    #include <shadowmask_pars_fragment>
            ...
            float max3(vec3 color) { return max(color.x, max(color.y, color.z)); }
            ...
            //float exposure = 1.5;
            //return pow(color * vec3(exposure) / (vec3(1.0) + color / vec3(exposure)), vec3(1.0 / 2.2)) ;

            // lumaBasedReinhardToneMapping
            /*float luma = dot(color, vec3(0.2126, 0.7152, 0.0722));
            float toneMappedLuma = luma / (1.0 + luma);
            return pow(color * toneMappedLuma / luma, vec3(1.0 / 2.2));*/
            ...
            //return	pow(exp(-1.0 / (2.72*color + 0.15)), vec3(1.0 / 2.2));
            ...
            metalness = GetIntensityFromLinearRGB(texColor);
            }*/
