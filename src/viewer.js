...
var pointerScaleVector = new THREE.Vector3();
var pointerScaleFactor = 35;
...
export default class ClosetViewer {
  constructor() {
    ...
    this.updateAnnotationPointerSize = this.updateAnnotationPointerSize.bind(this)
    ...
    this.computePointerPosition = this.computePointerPosition.bind(this)
  ...
  createAnnotation( pointerPos, faceNormal, cameraPos, message )
  {
    ...
    {
      var pointerPosition = AnnotationList[i].pointerPos;
  ...
  intersectObject(mouse = { x: 0, y: 0 }) {
    ...
    {
        raycaster.setFromCamera(mouse, this.camera);
        var intersects = raycaster.intersectObjects(this.annotationPointerList, true);
        ...
        if ( intersects.length > 0 )
        {
            ...
            {
                if(intersects[0].object === AnnotationList[i].sprite)
                {
                    var to = {
                        x: AnnotationList[i].cameraPos.x,
                        y: AnnotationList[i].cameraPos.y,
                        z: AnnotationList[i].cameraPos.z
                    };
                    ...
                    if(this.camera.position.x !== to.x || this.camera.position.y !== to.y || this.camera.position.z !== to.z )
                    {
                        ...
                        endQuaternion.copy(AnnotationList[i].cameraQuaternion);
                        endQuaternion.normalize();

                        var target = new THREE.Vector3();
                        target.copy(AnnotationList[i].cameraTarget)
                        ...
                        var tween = TweenMax.to(this.camera.position, 3, { x: to.x, y: to.y, z: to.z, ease:Circ.easeInOutQuad, onUpdate:onUpdate});
                    }
                }
            }
  ...
  updateAnnotationPointerSize()
  {
      for (var i = 0 ; i < AnnotationList.length; i++)
      {
          var scale = pointerScaleVector.subVectors(AnnotationList[i].sprite.position, this.camera.position).length() / pointerScaleFactor;
          AnnotationList[i].sprite.scale.set(scale/2, scale/2, 1); 
      }
  }
  ...
  render() {
    ...
    this.updateAnnotationPointerSize(); // update annotation pointer size
    //
